...
def condResizeFn_true(ms_modulation_trans, ms_modulation_phase, parameters):
    ...
    resizeTo = tf.cond(radial_flag, lambda: [1, calc_samplesM["r"]], lambda: [calc_samplesM["y"], calc_samplesM["x"]],)

    # Resize transmittance of the field -- Just upsampling so nearest interp req.
    calc_modulation_trans = tf.image.resize(
        ms_modulation_trans, resizeTo, method=method, preserve_aspect_ratio=False, antialias=False,
    )

    # Resize phase of the field -- Just upsampling so nearest interp req.
    calc_modulation_phase_real = tf.image.resize(
        tf.cos(ms_modulation_phase), resizeTo, method=method, preserve_aspect_ratio=False, antialias=False,
    )
    calc_modulation_phase_imag = tf.image.resize(
        tf.sin(ms_modulation_phase), resizeTo, method=method, preserve_aspect_ratio=False, antialias=False,
    )
...
def regularize_ms_calc_tf(
    ...
    Returns:
        `tf.float64`: Upsampled and padded metasurface transmittance of shape (batch_size, calc_samplesN['y'], calc_samplesN['x'])
            or (batch_size, 1, calc_samplesN['r']).
        `tf.float64`: Upsampled and padded metasurface phase of shape (batch_size, calc_samplesN['y'], calc_samplesN['x'])
            or (batch_size, 1, calc_samplesN['r'])
    """
    ...
    resizeCondition = tf.math.logical_or(
        tf.greater(calc_samplesM["x"], ms_samplesM["x"]), tf.greater(calc_samplesM["y"], ms_samplesM["y"]),
    )
    ...
    padCondition = tf.math.logical_or(
        tf.greater(calc_samplesN["x"], calc_samplesM["x"]), tf.greater(calc_samplesN["y"], calc_samplesM["y"]),
    )
    calc_modulation_trans, calc_modulation_phase = tf.cond(
        padCondition,
