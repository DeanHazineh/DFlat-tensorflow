...
#     # Run the conovlution with padding: Pad equal to the initial filter shape
#     init_image_shape = image.shape
#     im_ny = init_image_shape[-2]
#     im_nx = init_image_shape[-1]
#     init_filter_shape = filter.shape
...
#     # # First make sure that the image has odd dimensions, if not then pad so the FT is well defined
#     # # Filter will be resized via padding to patch the image dimensions
#     # init_image_shape = image.shape
#     # im_ny = init_image_shape[-2]
#     # im_nx = init_image_shape[-1]
#     # padding = np.zeros((len(init_image_shape), 2))

#     # padding[-2, :] = [im_ny, im_ny + 1] if np.mod(im_ny, 2) == 0 else [im_ny, im_ny]
#     # padding[-1, :] = [im_nx, im_nx + 1] if np.mod(im_nx, 2) == 0 else [im_nx, im_nx]
#     # image = tf.pad(image, padding)
...
#     filter_shape = tf.constant(filter.shape)
#     filter_resh = tf.reshape(
#         tf.image.resize_with_crop_or_pad(tf.transpose(filter, [-1, filter_shape[-2], filter_shape[-1], 1]), image_shape[-2], image_shape[-1]),
#         tf.concat([filter_shape[:-2], image_shape[-2:]], axis=0),
#     )
