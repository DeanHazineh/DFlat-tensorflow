...
def lookup_D2_pol2(phaseTable, transTable, p1_vect, p2_vect, wavelength, use_wavelength, ms_trans, ms_phase):
    ...
    or_table = np.hstack((np.expand_dims(or_table[0:1, :, :].flatten(), -1), np.expand_dims(or_table[1:2, :, :].flatten(), -1)))

    # Get the two polarization profiles for each metasurface
    ms_trans = np.hstack(
        (
            np.expand_dims(ms_trans[0:1, :, :].flatten(), -1),
            np.expand_dims(ms_trans[1:2, :, :].flatten(), -1),
        )
    )
    ms_phase = np.hstack(
        (
            np.expand_dims(ms_phase[0:1, :, :].flatten(), -1),
            np.expand_dims(ms_phase[1:2, :, :].flatten(), -1),
        )
    )
    ...
    for cell in range(target_profile.shape[0]):
        _, param_idx = min(
            ...
            for (idx, val) in enumerate(
                0.5 * np.abs(target_profile[cell, 0] - or_table[:, 0]) + 0.5 * np.abs(target_profile[cell, 1] - or_table[:, 1])
            )
        )
