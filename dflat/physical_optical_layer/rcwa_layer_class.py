...
class RCWA_Layer(tf.keras.layers.Layer):
    ...
    def __init__(self, rcwa_parameters):
        ...
        # batch wavelength will be deprecated since its so inneficient, not worth it
    ...
    def __call__(self, norm_param):
        ...
        field = self.rcwa_caller(norm_param, self.rcwa_parameters)
...
class RCWA_Latent_Layer(RCWA_Layer):
    ...
    def __init__(self, rcwa_parameters):
        ...
        `rcwa_parameters` (rcwa_param): Configuration dictionary object providing the rcwa solver settings.
        """
        super(RCWA_Latent_Layer, self).__init__(rcwa_parameters)
